...
async function importNotes() {
  ...
  try {
    ...
    // Transform old notes to match MongoDB format
    const notesToInsert = oldNotes.map((note) => {
      ...
      // If you want to preserve the original id as a field (optional)
      // mongoNote.originalId = note.id;

      return mongoNote;
    });

    console.log(`Preparing to import ${notesToInsert.length} notes...`);

    // Insert all notes
    const result = await collection.insertMany(notesToInsert);

    console.log(`✅ Successfully imported ${result.insertedCount} notes!`);
    console.log("\nInserted IDs:");
